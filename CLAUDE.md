...
- **Bug reporting:** never show the external report widget's floating trigger on the marketing landing page. On desktop product routes, keep the floating trigger available. On mobile, never float it over product work surfaces or the bottom tab bar; launch the same report dialog from the dedicated Help & feedback action in Profile. Keep all mobile text-entry controls at 16px or larger so iOS Safari does not zoom the page on focus.
