...
export function guidanceEmailContent(stage: 1 | 2 | 3, context: GuidanceContext, unsubscribeUrl?: string) {
  if (stage === 1) {
  ...
  if (stage === 2) {
    ...
    const text = [title, "", intro, "", "A short, imperfect list tells Sage more than a giant list built from rankings.", "", `${button}: ${href}`, "", "— Sage", "", `Manage email preferences: ${APP_URL}/profile`, ...(unsubscribeUrl ? [`Unsubscribe: ${unsubscribeUrl}`] : [])].join("\n");
    return { subject, html, text };
  }
...
export function weeklyDigestEmailContent(context: DigestContext, unsubscribeUrl?: string) {
  ...
  const text = [title, "", ...textSchools, ...(context.totalSchoolCount > context.schools.length ? [`Plus ${context.totalSchoolCount - context.schools.length} more in My Schools.`] : []), ...textDeadlines, "", `For this week: ${nextStep}`, "", `Open My Schools: ${APP_URL}/profile`, "", `Manage email preferences: ${APP_URL}/profile`, ...(unsubscribeUrl ? [`Unsubscribe: ${unsubscribeUrl}`] : [])].join("\n");
  return { subject, html, text };
}
